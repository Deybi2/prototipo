...
export default function PantallaRegistro() {
  ...
  const { cargando, error, setError, register, verificarConexion } = useAuthPocketbase()
  ...
  const manejarEnvio = async (e: React.FormEvent) => {
    e.preventDefault()
    const ok = await register(formulario.nombreUsuario, formulario.email, formulario.contraseña)
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-indigo-950 via-indigo-900 to-indigo-800 flex items-center justify-center p-4">
      <section className="w-full max-w-md rounded-3xl bg-white/95 shadow-2xl border border-indigo-100 p-6 md:p-8">
        ...
        <h1 className="text-2xl font-bold text-indigo-950 mb-1">Crear cuenta</h1>
        <p className="text-indigo-700 text-sm mb-6">Registro seguro conectado a PocketBase.</p>
        ...
        <form onSubmit={manejarEnvio} className="space-y-4">
          <div>
            ...
            <Input id="nombreUsuario" required minLength={3} value={formulario.nombreUsuario} onChange={(e) => { setFormulario((p) => ({ ...p, nombreUsuario: e.target.value })); setError(null) }} className="mt-1 border-indigo-200 focus-visible:ring-indigo-500" />
          </div>
          <div>
          ...
          <div>
            ...
            <Input id="contraseña" type="password" required minLength={6} value={formulario.contraseña} onChange={(e) => { setFormulario((p) => ({ ...p, contraseña: e.target.value })); setError(null) }} className="mt-1 border-indigo-200 focus-visible:ring-indigo-500" />
          </div>
          ...
          <Button type="submit" disabled={cargando || conectado === false} className="w-full h-12 rounded-xl bg-indigo-600 hover:bg-indigo-700 shadow-md hover:shadow-lg transition-all">
            {cargando ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Creando…</> : "Registrarme"}
          </Button>
        </form>
      </section>
    </main>
